...
//        SetCameraTarget( (N2-2)*dd/2, (N3-1)*dd/2-.5, -(N1-1)*dd/2 )           
//        IntelligentEyeT( -53, 38, 140)       
//        CameraLights(15, 4, .1, 60, 30 )
        #break                       

    #case (_TextreFunc)  
       #warning "Vis. by texture map. Not fully implemented yet, sorry :/ \n" // 4safety        
    #local gMax=10;
    SetGradientAccuracy(.0001)     
    // PotentialMapG(  fn_Gradient(function{clip(log(VVV(x,y,z)), 0, gMax )/gMax}), y, .0 )      
    PotentialMapG(  function{clip(log(VVV(x,y,z)), 0, gMax )/gMax}, y, .0 )      
