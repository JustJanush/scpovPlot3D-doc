...
////===================
    ...
    #local grV = function{ clip(log(grV0(x,y,z)-gMin)-gMin2, 0, gMax-gMin2)/(gMax-gMin2) };
      SetIsoTexture( texture{ 
       ...
       #declare ISO3 = MakeEquiPlane( VVV, -.5, BoxMin, BoxMax ) // zero plane
       object{ISO3}
    ...
    SetGradientAccuracy(.0001)      
        ...
        #while (_num>0)  
           ...
           #local zr = az*rand(R3)-8;  
               ...
               #local p0 = p0+(Vtrsh-VVV(p0.x, p0.y, p0.z))*nvGrd/vlength(vGrd); // ..compute and apply correction to the point 'p0'
               ...
               sphere{
                    p0, clip(abs(Vtrsh-VVV(p0.x, p0.y, p0.z)), .1, .25) //max(0.05, rr/50)
                    texture{ pgmnt }
               }
               #local _num=_num-1;
          #end                    
