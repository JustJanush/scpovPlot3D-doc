...
#local Nlin = 15;                 
#local Npts = 1700; //frame_number+1;
...
#for(_ai, 1, Nlin)
    #local alpha0 = 52.225 + (_ai-1)*5;  // degrees
...
#local dst_ = 15; // distance from observed point
...
#local ther = radians(the_); 
#local V1 = 1.85*<cos(alfr),0,-sin(alfr)>    // left|right
#local V2 = 0.70*<sin(alfr), 0,  cos(alfr)>  // top|bottom
#local V3 = 1.00*vnormalize(V1+V2-<camera_x, camera_y, camera_z>);
...
CameraOn(dst_)
//IntelligentEyeT( alf_, the_, dst_)
