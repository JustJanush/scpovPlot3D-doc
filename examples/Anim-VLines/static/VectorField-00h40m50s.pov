...
background {rgbt <.0150, .0150, .0150, 0>}
...
* SEE ALSO
*  DrawStreamLines, ComputeStreamline
*   
******/
...
// _P0 start point, _Npts 
// lets make streamline! _Lnum
/****M* VectorField.inc/ComputeStreamline
* PURPOSE
*  this macro computes nodes only of single streamline
*  for given line index, using Runge-Kutta 4th order algorithm
*  
* SYNOPSIS
*/
...
*  integer  _Npts - expected number of nodes, 
...
******/
    ...
    #declare StrmLineTable[_Lnum][1][0] = rr; //rrr*vlength(VV); // vector length [V/m]=>radius of given node of streamline
    ...
    #for(ind, 2, _Npts) // Runge-Kutta of 4 order           
        #local _vn = vlength( _VV ); // unary vector parallel to local vector field
        #if ((ind > 10) & ((_vn< _vnMin) | (_vn >_vnMax)) )
            ...
            #declare StrmLineTable[_Lnum][ind][0] = rr; //rrr*vlength(VV); // vector length [V/m]=>radius of given node of streamline
...
#declare rr  = .02; //.025; // line diameter
#declare rrc = .035;
#declare rrr = 20000;

#local Dalp  = 0; // degrees
#local Dalp  = radians(Dalp); // radians
#local radii = (0.75)*HOdist; //1.25
#local ymin  = 0;
#local ymax  = .5;  
#local yy    = .13; // ymin+clock*(ymax-ymin);

DeclareStructure(H2O_Atoms)                                                     

#local Nlin = 19;                 
#local Npts = 50; //frame_number+1;
#declare StrmLineTable = array[Nlin+1][Npts+1][4]; // [0][0][0] - num of streamlines, [0][*][*] - reserved
                                                   // [k][0][0] - num of points in `k`-th streamline, may depend on field's structure n<=Npts   

#declare StrmLineTable[0][0][0] = Nlin;
#local _vnMax = 100;
#local _vnMin = 0;
// control points

#local _h = .2086329; // Runge-Kutty step
#for(_ai, 1, Nlin)
    #local alpha0 = 55 + (_ai-1)*11;  // degrees
    #local alpha0 = radians(alpha0); // rad
    #declare P0 = radii*<cos(alpha0), yy, -sin(alpha0)>;  // test: cylinder{P00,VV,StrmLineTable[1][0]*1000 texture{pigment{color rgb<1.0, .3,  .3>} finish{phong .7 reflection metallic}}}
    ComputeStreamline(P0, Npts, _ai) 
#end

DrawStreamLines( StrmLineTable, 3, _IsoTexture )   

#local _h = -.2086329; // Runge-Kutty step
#for(_ai, 1, Nlin)
    #local alpha0 = 55 + (_ai-1)*11;  // 52.225 degrees
    #local alpha0 = radians(alpha0); // rad
    #declare P0 = radii*<cos(alpha0), yy, -sin(alpha0)>;  // test: cylinder{P00,VV,StrmLineTable[1][0]*1000 texture{pigment{color rgb<1.0, .3,  .3>} finish{phong .7 reflection metallic}}}
    ComputeStreamline(P0, Npts, _ai) 
#end
...
// vignette
#local alf_ = 81;   // azimuth relative to -<Z>_p axis
#local the_ = 57;   // elevation relative to Z-X plane                          
#local dst_ = 5.5;  // distance from observed point
...
#local VP  = vcross(y, VCT);       // horizontal r-hand vector   
  ...
  #local VUD = vcross(VP, VCT);      // Up-Down vector
#else            
  #local VP  = x;
  #local VUD = -z;
#end  
...
// automated section
#local V3 = VC + 15*vnormalize(VCT+.06*VUD); // in front of Camera
