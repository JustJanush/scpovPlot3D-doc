...
#declare Molecule = object{CreateStruct( H2O_Atoms, H2O_Bonds, 0.05)}; // PTube, Bonds, .05)};
object{Molecule} 

#declare BoxMin   = 6*min_extent(Molecule);   
#declare BoxMax   = 6*max_extent(Molecule);
                                       
#declare BoxMin   =  <BoxMin.x, BoxMin.y*1.2, BoxMin.z>;
//                                       ^----- tune this coeeficient manually!
#declare BoxMax   =  <BoxMax.x, BoxMax.y/10, BoxMax.z>;
//                                       ^----- ..as well as this one
  ...
  box{ BoxMin, BoxMax 
       ...
       finish{reflection metallic diffuse .3 ambient .93 phong .3}
}           
#end

//#declare VVV = CreateVLJ(PTube)
#declare VVV  = CreateVFC(H2O_Atoms)   // VVV() - without scaling factor
#declare VVV3 = CreateVFC2(H2O_Atoms)  // VVV() - WITH scaling factor

SetAccuracy(.02)
SetMaxGrad(5)

SetIsoTexture( texture{ pigment{color rgbt<.9, .9, .9, .3>}} )
#declare ISO3 = MakeEquiPlane( VVV, 0, BoxMin, BoxMax ) // zero plane
object{ISO3}


SetIsoTexture( texture{ pigment{color rgbt<0,.1,1,.3>}} )
...
SetIsoTexture( texture{ pigment{color <1,0.1,0>}} )
#declare ISO4 = MakeEquiPlane( VVV, 0.07/500000, BoxMin, BoxMax )

SetIsoTexture( texture{ pigment{color rgbt<1,0.1,0,.3>}} )
#declare ISO5 = MakeEquiPlane( VVV, 0.035/500000, BoxMin, BoxMax )

object{ISO4}
object{ISO5}


SetIsoTexture( texture{ pigment{color rgbt<1, 0.1, 0.1,.5>}} )
#declare ISO6 = MakeEquiPlane( VVV, 0.01/500000, BoxMin, BoxMax )

object{ISO6}

SetIsoTexture( texture{ pigment{color rgbt<.1, 0.1, 1,.5>}} )
#declare ISO7 = MakeEquiPlane( VVV, -0.01/500000, BoxMin, BoxMax )

object{ISO7}



//PotentialMap(  VVV3, y, .0,     7500000 )      
//PotentialLines(VVV3, y, .0, 20, 500000 )
...
background {rgbt <.0031, .0031, .0031, 0>}

light_source { < -10,  30, -10> 1 }
light_source { <  10,  30, -10> 1 }
...
SetCameraTarget( .0, -.85, -.80)           
IntelligentEyeT( 13, 40, 13)       
